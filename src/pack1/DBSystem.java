...
public class DBSystem {
  ...
  void fail(int siteId) { // a site fails
    ...
    Site site = Site.getSite(this.sites,siteId);
    HashMap<String , ArrayList<Lock>> copy_lock_table = new HashMap<String , ArrayList<Lock>>();
    ...
    while(keySetIterator.hasNext()){
      ...
      copy_lock_table.put(var, lockList);
    }

    Iterator<String> other_keySetIterator = copy_lock_table.keySet().iterator();
    while(other_keySetIterator.hasNext()){
      String var = other_keySetIterator.next();
      ArrayList<Lock> lockList = copy_lock_table.get(var);
